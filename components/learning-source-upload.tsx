...
export function LearningSourceUpload({ disabled, cloudUserId, onQueueBundle, onDebug }: Props) {
  ...
  const selectFiles = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (selected.length + files.length > MAX_LESSON_SOURCES) {
      setFailure(`Lessons support a maximum of ${MAX_LESSON_SOURCES} source files.`);
      return;
    }
    ...
    if (invalid) {
      setFailure(!SUPPORTED_SOURCE_TYPES.includes(invalid.type as SupportedSourceType) ? `${invalid.name} is not a PDF or TXT file.` : !invalid.size ? `${invalid.name} is empty.` : `${invalid.name} exceeds the ${formatMebibytes(MAX_SOURCE_BYTES)} per-source limit.`);
      return;
    }
    const accepted = files.map((file): SelectedSource => ({
      file,
  ...
  const bundleLimitFailure = totalBytes > bundleLimit
    ? cloudUserId
      ? `Signed-in lessons support up to ${formatMebibytes(MAX_CLOUD_SOURCE_BUNDLE_BYTES)} of source files in total.`
      : `Signed-out lessons support up to ${formatMebibytes(MAX_SOURCE_BUNDLE_BYTES)} of source files in total. Sign in to use up to ${formatMebibytes(MAX_CLOUD_SOURCE_BUNDLE_BYTES)}.`
    : null;
  const displayedFailure = bundleLimitFailure ?? failure;
  return <section className="source-upload" aria-labelledby="source-upload-title">
    ...
    <div className="upload-step-heading"><p className="step-label">2 · Add learning materials</p><p>{cloudUserId ? `Up to ${formatMebibytes(MAX_SOURCE_BYTES)} per file · ${formatMebibytes(MAX_CLOUD_SOURCE_BUNDLE_BYTES)} total · Maximum ${MAX_LESSON_SOURCES} files.` : `Up to ${formatMebibytes(MAX_SOURCE_BUNDLE_BYTES)} total while signed out · Maximum ${MAX_LESSON_SOURCES} files. Sign in for up to ${formatMebibytes(MAX_CLOUD_SOURCE_BUNDLE_BYTES)} total.`}</p></div>
    ...
    {displayedFailure && <p className="source-error" role="alert">{displayedFailure}</p>}
