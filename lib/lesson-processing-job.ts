...
export type ProcessingJobStatus = "queued" | "uploading" | "processing" | "saving" | "ready" | "error" | "needs-source";
export type ProcessingJobSource = LessonSource;
export type LessonProcessingJob = {
  id: string;
  lessonId: string;
  ownerId: string | null;
  title: string;
  sources: ProcessingJobSource[];
  status: ProcessingJobStatus;
  phase: string;
  createdAt: string;
  updatedAt: string;
  error?: { category: SourceProcessingErrorCode | "UPLOAD_FAILED" | "SAVE_FAILED"; message: string; retryable: boolean };
  generation: number;
};

const STATUSES = new Set<ProcessingJobStatus>(["queued", "uploading", "processing", "saving", "ready", "error", "needs-source"]);

export async function listProcessingJobs(ownerId: string | null) {
  const database = await openTutorDatabase();
  try {
    const values = await requestResult<unknown[]>(database.transaction(PROCESSING_JOBS_STORE, "readonly").objectStore(PROCESSING_JOBS_STORE).getAll());
...
function parseProcessingJob(value: unknown): LessonProcessingJob | null {
  ...
  if (!isUuid(job.id) || !isUuid(job.lessonId) ||
      ...
      !Array.isArray(job.sources) || !job.sources.length || !STATUSES.has(job.status as ProcessingJobStatus) ||
      typeof job.phase !== "string" || !isDate(job.createdAt) || !isDate(job.updatedAt) ||
