...
export async function createQueuedSourceUpload(
  ...
  const { user } = await authenticatedClient(expectedUserId);
  const config = getSupabasePublicConfig();
  if (!config || user.id !== expectedUserId || !isUuid(lessonId) || !isUuid(source.id)) {
    throw new Error("cloud-source-upload-invalid");
  }
  const client = createSupabaseBrowserClient();
  const { data, error } = client ? await client.auth.getSession() : { data: { session: null }, error: new Error("unavailable") };
...
export async function upsertStoredLessonSources(
  lesson: SavedLesson,
  expectedUserId: string,
  debug?: (message: string) => void,
) {
  ...
  const rows = lesson.sources
    .filter((source) => source.storageStatus === "stored" && source.storagePath)
    .map((source) => ({
      id: source.id,
      lesson_id: lesson.id,
      user_id: user.id,
...
export async function deletePreLessonSourceObjects(paths: string[], expectedUserId: string) {
  ...
  const { client, user } = await authenticatedClient(expectedUserId);
  if (paths.some((path) => path.split("/")[0] !== user.id)) {
    throw new Error("cloud-source-path-invalid");
  }
